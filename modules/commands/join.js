...
module.exports.handleReply = async ({ event, api, handleReply, Threads }) => {
    ...
    if (!threadList[parseInt(body) - 1]) return api.sendMessage("[ 🐳 ] 𝐋𝐮̛̣𝐚 𝐜𝐡𝐨̣𝐧 𝐜𝐮̉𝐚 𝐛𝐚̣𝐧 𝐤𝐡𝐨̂𝐧𝐠 𝐧𝐚̆̀𝐦 𝐭𝐫𝐨𝐧𝐠 𝐝𝐚𝐧𝐡 𝐬𝐚́𝐜𝐡.", threadID, messageID);
        ...
        try {
            ...
            if (participantIDs.includes(senderID)) return api.sendMessage('[ 🐳 ] 𝐁𝐚̣𝐧 đ𝐚̃ 𝐜𝐨́ 𝐦𝐚̣̆𝐭 𝐭𝐫𝐨𝐧𝐠 𝐧𝐡𝐨́𝐦 𝐧𝐚̀𝐲.', threadID, messageID);
            api.addUserToGroup(senderID, threadInfo.threadID, (e) => {
              if (e) api.sendMessage(`[ 🐳 ] Đ𝐚̃ 𝐱𝐚̉𝐲 𝐫𝐚 𝐥𝐨̂̃𝐢: ${e.errorDescription}`, threadID, messageID);
              else api.sendMessage(`[ 🐳 ] 𝐀𝐧𝐝𝐫𝐞𝐞 𝐯𝐮̛̀𝐚 𝐭𝐡𝐞̂𝐦 𝐛𝐚̣𝐧 𝐯𝐚̀𝐨 𝐧𝐡𝐨́𝐦 ${threadInfo.name} 𝐫𝐨̂̀𝐢 𝐧𝐡𝐚. 𝐊𝐢𝐞̂̉𝐦 𝐭𝐫𝐚 𝐨̛̉ 𝐦𝐮̣𝐜 𝐬𝐩𝐚𝐦 𝐡𝐨𝐚̣̆𝐜 𝐭𝐢𝐧 𝐧𝐡𝐚̆́𝐧 𝐜𝐡𝐨̛̀ 𝐧𝐞̂́𝐮 𝐤𝐡𝐨̂𝐧𝐠 𝐭𝐡𝐚̂́𝐲 𝐛𝐨𝐱 𝐧𝐡𝐚.`, threadID, messageID);
            });
        }
        catch (error) {
            return api.sendMessage(`[ 🐳 ] :( 𝐄𝐦 𝐛𝐢̣ 𝐥𝐨̂̃𝐢: ${error}`, threadID, messageID);
        }
    }
};
