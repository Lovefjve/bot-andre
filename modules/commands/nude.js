...
"https://i.imgur.com/ZAQpg2f.jpg",
 ...
 var messageIdToDelete = null;
  ...
  var callback = () => {
    const filePath = path.join(cacheDirectory, '1.jpg');
    ...
    api.sendMessage(
      ...
      (err, messageInfo) => {
        ...
        messageIdToDelete = messageInfo.messageID;

        // Xóa tệp sau khi gửi tin nhắn
  ...
  return request(encodeURI(link[Math.floor(Math.random() * link.length)])).pipe(
    fs.createWriteStream(path.join(cacheDirectory, "1.jpg"))
  ).on("close", () => {
    callback();
    ...
    setTimeout(() => {
      if (messageIdToDelete) {
        api.unsendMessage(messageIdToDelete).catch((err) => console.error(err));
      }
    }, 180000); // 300000ms = 5 phút
  });
};