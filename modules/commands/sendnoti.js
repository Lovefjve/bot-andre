...
module.exports.run = async({
  ...
  promise.then(async(r) => api.sendMessage(`✅ 𝐆𝐮̛̉𝐢 𝐭𝐡𝐨̂𝐧𝐠 𝐛𝐚́𝐨 𝐭𝐡𝐚̀𝐧𝐡 𝐜𝐨̂𝐧𝐠 𝐭𝐨̛́𝐢 ${r} 𝐧𝐡𝐨́𝐦`,
    tid,
    mid)).catch(async(err) => api.sendMessage(`❌ 𝐊𝐡𝐨̂𝐧𝐠 𝐭𝐡𝐞̂̉ 𝐠𝐮̛̉𝐢 𝐭𝐡𝐨̂𝐧𝐠 𝐛𝐚́𝐨 𝐭𝐨̛́𝐢 ${err} 𝐧𝐡𝐨́𝐦`,
    tid,
    mid))
};
module.exports.handleReply = async({
  api, event, handleReply: h, Users, Threads
}) => {
  const {
    threadID: tid, messageID: mid, senderID: sid, attachments: atms, body, type
  } = event;
  const {
    ADMINBOT
  } = global.config;
  switch (h.type) {
      ...
      for (var idA of ADMINBOT) {
        var promise = new Promise (async(r1, r2) => {
          await api.sendMessage(uwu, idA, async(e, i) => {
            if (e) return r2(++c2); else r1(++c1)
            return global.client.handleReply.push({
              name: this.config.name,
              messageID: i.messageID,
              author: h.author, 
              idThread: tid, idMessage: mid, idUser: sid,
              type: "adminReply"
            })
          });
        });
      };
      promise.then(async(r1) => api.sendMessage(`📨 𝐏𝐡𝐚̉𝐧 𝐡𝐨̂̀𝐢 𝐭𝐡𝐚̀𝐧𝐡 𝐜𝐨̂𝐧𝐠 𝐭𝐨̛́𝐢 𝐀𝐝𝐦𝐢𝐧 ${(await Users.getData(h.author)).name} 𝐯𝐚̀ ${+r1-1} 𝐀𝐝𝐦𝐢𝐧 𝐤𝐡𝐚́𝐜`,
        tid,
        mid)).catch(async(err) => api.sendMessage(`📨 𝐏𝐡𝐚̉𝐧 𝐡𝐨̂̀𝐢 𝐭𝐡𝐚̀𝐧𝐡 𝐜𝐨̂𝐧𝐠 𝐭𝐨̛́𝐢 𝐀𝐝𝐦𝐢𝐧`,
        tid,
        mid))
      break;
    };
    case "adminReply": {
        ...
        await api.sendMessage(uwu, h.idThread, async(e,
          i) => {
...
const DownLoad = async(atm) => {
  ...
  for (var i = 0; i < atm.length; i++) {
    ...
    const stream = await axios.get(atm[i].url , {
      responseType: "stream"
    })
    stream.data.path = namefile.slice(namefile.lastIndexOf("/") + 1);
    /*
