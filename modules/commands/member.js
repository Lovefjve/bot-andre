...
module.exports.handleReply = async function ({ api, event, handleReply }) {
	...
	if (args[0] == 'all') {
		...
		if (threadInfo.adminIDs.find(item => item.id == api.getCurrentUserID()) == undefined)
			return api.sendMessage('𝗕𝗼𝘁 𝗰𝗮̂̀𝗻 𝗤𝗧𝗩 𝗰𝗵𝗼 𝘃𝗶𝗲̣̂𝗰 𝗹𝗼̣𝗰 𝗺𝗲𝗺, 𝘃𝘂𝗶 𝗹𝗼̀𝗻𝗴 𝗰𝗮̂́𝗽 𝗾𝘁𝘃 𝗿𝗼̂̀𝗶 𝘁𝗵𝘂̛̉ 𝗹𝗮̣𝗶 𝘀𝗮𝘂 𝗻𝗵𝗲́ 💕', event.threadID, event.messageID);
		...
		return api.sendMessage(`𝗧𝗵𝘂̛̣𝗰 𝘁𝗵𝗶 𝘅𝗼𝗮́ 𝘁𝗵𝗮̀𝗻𝗵 𝗰𝗼̂𝗻𝗴 ${success.length} 𝘁𝗵𝗮̀𝗻𝗵 𝘃𝗶𝗲̂𝗻\n${fail.length > 0 ? `𝗞𝗵𝗼̂𝗻𝗴 𝘁𝗵𝗲̂̉ 𝘅𝗼́𝗮 ${fail.length} 𝘁𝗵𝗮̀𝗻𝗵 𝘃𝗶𝗲̂𝗻: ${fail.map(item => item.name).join(', ')}` : ''}`, event.threadID, event.messageID);
	}
	else {
		const indexs = args.filter(item => !isNaN(item) && parseInt(item) >= 1 && parseInt(item) <= handleReply.data.length);
		if (indexs.length == 0)
			return api.sendMessage('𝗞𝗵𝗼̂𝗻𝗴 𝗰𝗼́ 𝘁𝗵𝗮̀𝗻𝗵 𝘃𝗶𝗲̂𝗻 𝗻𝗮̀𝗼 𝗯𝗶̣ 𝗰𝗵𝗼̣𝗻', event.threadID, event.messageID);
...
module.exports.run = async function ({
	...
	if (args[0] == 'set') {
		...
		if (isNaN(day))
			return api.sendMessage('𝗦𝗼̂́ 𝗻𝗴𝗮̀𝘆 𝗸𝗵𝗼̂𝗻𝗴 𝗵𝗼̛̣𝗽 𝗹𝗲̣̂', event.threadID, event.messageID);
		const pathData = `${dir}/${event.threadID}.json`;
		if (!fs.existsSync(pathData))
			fs.writeFileSync(pathData, JSON.stringify({
				dayNumber: day,
		...
		return api.sendMessage(`𝗧𝗵𝗶𝗲̂́𝘁 𝗹𝗮̣̂𝗽 𝘁𝗵𝗮̀𝗻𝗵 𝗰𝗼̂𝗻𝗴`, event.threadID, event.messageID);
	}
	else {
