...
module.exports = class Role {
	...
	request(ability, time = gameConfig.timeout[ability.name] || 30000) {
		return new Promise(resolve => {
			const timeoutID = setTimeout(() => {
				this.sendMessage('𝗛𝗲̂́𝘁 𝘁𝗵𝗼̛̀𝗶 𝗴𝗶𝗮𝗻 𝗿𝗼̂̀𝗶 ⌛');
				this.resolve([null, null]);
			}, time);
		...
		for (let index = 0; index < formats.length; index++) {
			...
			if (Array.isArray(formats[index])) {
				if (!formats[index].includes(value))
					throw new Error(
						`𝗩𝘂𝗶 𝗹𝗼̀𝗻𝗴 𝗻𝗵𝗮̣̂𝗽 𝟭 𝘁𝗿𝗼𝗻𝗴 𝗰𝗮́𝗰 𝗴𝗶𝗮́ 𝘁𝗿𝗶̣ 𝘀𝗮𝘂: ${formats[index].join(
							', '
						)}!`
					);
			} else {
	...
	async die(death) {
		this.died = true;
		let rep =
			`𝗖𝗵𝘂́𝗻𝗴 𝘁𝗼̂𝗶 𝘃𝗼̂ 𝗰𝘂̀𝗻𝗴 𝘁𝗵𝘂̛𝗼̛𝗻𝗴 𝘁𝗶𝗲̂́𝗰 𝗯𝗮́𝗼 𝘁𝗶𝗻: ${this.name} (𝗯𝗮̣𝗻) ` +
		...
		case DeathType.P2P:
			if (death.killer != death.victim)
				rep += `𝗡𝗴𝘂𝘆𝗲̂𝗻 𝗻𝗵𝗮̂𝗻 𝗰𝗵𝗲̂́𝘁: 𝗕𝗶̣ ${death.killer.name} (${death.killer.constructor.name}) 𝗴𝗶𝗲̂́𝘁 🩸\n`;
			else rep += '𝗡𝗴𝘂𝘆𝗲̂𝗻 𝗻𝗵𝗮̂𝗻 𝗰𝗵𝗲̂́𝘁: 𝗧𝘂̛̣ 𝘁𝘂̛̉';
			break;
		case DeathType.GANG:
			rep += `𝗡𝗴𝘂𝘆𝗲̂𝗻 𝗻𝗵𝗮̂𝗻 𝗰𝗵𝗲̂́𝘁: 𝗕𝗮̆𝗻𝗴 𝗵𝗼̣̂𝗶 ${death.killer.constructor.name} 𝗵𝗶𝗲̂́𝗽 𝗱𝗮̂𝗺 𝘁𝗮̣̂𝗽 𝘁𝗵𝗲̂̉ 𝘁𝗼̛́𝗶 𝗰𝗵𝗲̂́𝘁 ⚰️\n`;
			break;
		case DeathType.LYNCH:
			rep += '𝗡𝗴𝘂𝘆𝗲̂𝗻 𝗻𝗵𝗮̂𝗻 𝗰𝗵𝗲̂́𝘁: 𝗗𝗮̂𝗻 𝗹𝗮̀𝗻𝗴 𝘁𝗿𝗲𝗼 𝗰𝗼̂̉\n';
			break;
		case DeathType.SIMP:
			rep += `𝗡𝗴𝘂𝘆𝗲̂𝗻 𝗻𝗵𝗮̂𝗻 𝗰𝗵𝗲̂́𝘁: 𝗖𝗵𝗲̂́𝘁 𝘁𝗵𝗲𝗼 𝗻𝗴𝘂̛𝗼̛̀𝗶 𝘆𝗲̂𝘂 ${this.waifu.name}\n`;
			break;
		default:
			rep += '𝗡𝗴𝘂𝘆𝗲̂𝗻 𝗻𝗵𝗮̂𝗻 𝗰𝗵𝗲̂́𝘁: 𝗖𝗵𝘂̛𝗮 𝗿𝗼̃\n';
		}
		rep += '𝗟𝘂̛𝘂 𝘆́: 𝗛𝗮̃𝘆 𝗶𝗺 𝗹𝗮̣̆𝗻𝗴 𝘃𝗮̀ 𝘅𝗲𝗺 𝘁𝗶𝗲̂́𝗽 𝘁𝗿𝗼̀ 𝗰𝗵𝗼̛𝗶 𝗸𝗵𝗼̂𝗻𝗴 𝗰𝗵𝗮𝘁 𝗴𝗶̀ 𝗰𝗮̉\n';
