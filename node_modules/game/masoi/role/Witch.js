		...
		if (result.indexKill != -1 && this.potion.save) {
			...
			requests.push(
				await this.request({
					question() {
						return (
							`𝗧𝗼̂́𝗶 𝗻𝗮𝘆 ${victim.name} 𝘀𝗲̃ 𝗯𝗶̣ 𝗹𝘂̃ 𝘀𝗼́𝗶 𝗰𝗮̆́𝗻, 𝗯𝗮̣𝗻 𝗰𝗼́ 𝗺𝘂𝗼̂́𝗻 𝘀𝘂̛̉ 𝗱𝘂̣𝗻𝗴 𝗯𝗶̀𝗻𝗵 [ 𝗰𝘂̛́𝘂 𝗻𝗴𝘂̛𝗼̛̀𝗶 ] 𝗸𝗵𝗼̂𝗻𝗴 💉?\n` +
							`${symbols[1]} 𝗖𝗼́ ♥\n` +
							`${symbols[2]} 𝗞𝗵𝗼̂𝗻𝗴 😈`
						);
					},
					check(player, value) {
						...
						player.sendMessage(
							choose
								? `𝗕𝗮̣𝗻 𝘀𝘂̛̉ 𝗱𝘂̣𝗻𝗴 𝗯𝗶̀𝗻𝗵 [ 𝗰𝘂̛́𝘂 𝗻𝗴𝘂̛𝗼̛̀𝗶 ] 𝗹𝗲̂𝗻 ${victim.name} 💉`
								: `𝗕𝗮̣𝗻 𝘃𝘂̛̀𝗮 𝗰𝗵𝗼̣𝗻 𝗸𝗵𝗼̂𝗻𝗴 𝗰𝘂̛́𝘂 ${victim.name} 😈`
						);
						return choose;
					},
			...
			requests.push(
				await this.request({
					...
					check(player, value) {
						...
						player.sendMessage(
							`𝗕𝗮̣𝗻 𝘃𝘂̛̀𝗮 𝗰𝗵𝗼̣𝗻 𝗴𝗶𝗲̂́𝘁 ${player.world.items[index].name} 🧪`
						);
						return index;
					},
