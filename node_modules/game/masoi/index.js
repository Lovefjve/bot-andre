...
module.exports = class MasoiGame extends Game {
	constructor(options = {}) {
		...
		if(options.param[0] == 'info') {
			...
			if(!options.param[1]) return sendMessage(`𝗟𝗲̣̂𝗻𝗵: ${prefix}𝗺𝗮𝘀𝗼𝗶 𝗶𝗻𝗳𝗼 [𝗠𝗮̃ 𝗹𝗮̀𝗻𝗴]`, this.threadID);
			if (!gameConfig.setups[indexVillage]) return sendMessage(`𝗞𝗵𝗼̂𝗻𝗴 𝘁𝗶̀𝗺 𝘁𝗵𝗮̂́𝘆 𝗹𝗮̀𝗻𝗴 𝘃𝗼̛́𝗶 𝗺𝗮̃ 𝘀𝗼̂́ ${symbols[options.param[0]]}!`, this.threadID);	
		...
		if (!options.param[0] || isNaN(indexVillage)) {
            var body = `🐺==== 「 𝐌𝐀 𝐒𝐎́𝐈 」 ====🐺\n━━━━━━━━━━━━━━\n\n𝗛𝘂̛𝗼̛́𝗻𝗴 𝗱𝗮̂̃𝗻 𝘁𝗮̣𝗼: ${prefix}𝗺𝗮𝘀𝗼𝗶 <𝗺𝗮̃ 𝗹𝗮̀𝗻𝗴>\n` +
		...
		if (!gameConfig.setups[indexVillage]) {
            return sendMessage(`𝗞𝗵𝗼̂𝗻𝗴 𝘁𝗶̀𝗺 𝘁𝗵𝗮̂́𝘆 𝗹𝗮̀𝗻𝗴 𝘃𝗼̛́𝗶 𝗺𝗮̃ 𝘀𝗼̂́ ${symbols[options.param[0]]}!`, this.threadID);
        }
		...
		this.sendMessage(
				...
				`• 𝗟𝘂̛𝘂 𝗬́: 𝗧𝗮̂́𝘁 𝗰𝗮̉ 𝗻𝗵𝘂̛̃𝗻𝗴 𝘂𝘀𝗲𝗿 𝘁𝗿𝘂̛𝗼̛́𝗰 𝗸𝗵𝗶 𝗻𝗵𝗮̂́𝗻 𝗺𝗲𝗽𝗹𝗮𝘆 𝘃𝗮̀𝗼 𝗴𝗮𝗺𝗲, 𝗵𝗮̃𝘆 𝗶𝗯 𝘃𝗼̛́𝗶 𝗻𝗶𝗰𝗸 𝗯𝗼𝘁 𝘁𝗿𝘂̛𝗼̛́𝗰. 𝗕𝘂̛𝗼̛́𝗰 𝗻𝗮̀𝘆 𝗰𝘂̛̣𝗰 𝗸𝗶̀ 𝗾𝘂𝗮𝗻 𝘁𝗿𝗼̣𝗻𝗴 𝗻𝗲̂́𝘂 𝗸𝗵𝗼̂𝗻𝗴 𝘀𝗲̃ 𝗸𝗵𝗼̂𝗻𝗴 𝘁𝗵𝗲̂̉ 𝗰𝗵𝗼̛𝗶\n` +
	...
	async onMessage(message, reply) {
		...
		if (message.body.toLowerCase() == 'end!') {
			if (message.senderID == this.masterID) {
				...
				if (this.state.getCurrent() == State.SETUP)
					await reply('🛠 𝗞𝗲̂́𝘁 𝘁𝗵𝘂́𝗰 𝗴𝗮𝗺𝗲 𝘁𝗵𝗮̀𝗻𝗵 𝗰𝗼̂𝗻𝗴 !');
			} else {
		...
		if (message.body.toLowerCase() == 'out!') {
			if(!this.participants.includes(message.senderID)) 
				return await this.sendMessage(`⚠️ 𝗕𝗮̣𝗻 𝗰𝗵𝘂̛𝗮 𝘁𝗵𝗮𝗺 𝗴𝗶𝗮 𝗴𝗮𝗺𝗲 𝗻𝗲̂𝗻 𝗸𝗵𝗼̂𝗻𝗴 𝘁𝗵𝗲̂̉ 𝗼𝘂𝘁!\n• 𝗧𝗶̀𝗻𝗵 𝘁𝗿𝗮̣𝗻𝗴 𝗴𝗮𝗺𝗲 𝗵𝗶𝗲̣̂𝗻 𝘁𝗮̣𝗶: ${this.participants.length}/${this.setup.roles.length}!`);
			if(message.senderID == this.masterID) 
				...
				for (const player of werewolfParty) {
					if (nameMap.length > 1)
						await player.sendMessage(
							`𝗡𝗵𝘂̛̃𝗻𝗴 𝗻𝗴𝘂̛𝗼̛̀𝗶 𝗰𝘂̀𝗻𝗴 𝗽𝗵𝗲 𝘃𝗼̛́𝗶 𝗯𝗮̣𝗻 𝗹𝗮̀ 🐺: ${nameMap
								.filter(name => name != player.name)
								.join(
									', '
								)}\n 𝗛𝗮̃𝘆 𝗹𝗶𝗲̂𝗻 𝗵𝗲̣̂ 𝘃𝗼̛́𝗶 𝗵𝗼̣ 𝘃𝗮̀ 𝘁𝗮̣𝗼 𝟭 𝘁𝗲𝗮𝗺𝘄𝗼𝗿𝗸 𝘁𝗼̂́𝘁 𝗻𝗵𝗮̂́𝘁 𝗻𝗵𝗲́ !`
						);
				}
				let balanceScore = 0;
				...
				this.sendMessage(
					this.timing({
						message:
							...
							'\n𝗛𝗮̃𝘆 𝘅𝗲𝗺 𝗸𝗶̃ 𝗰𝗵𝗶 𝘁𝗶𝗲̂́𝘁 𝘃𝗮𝗶 𝘁𝗿𝗼̀ 𝗰𝘂̉𝗮 𝗺𝗶̀𝗻𝗵, 𝘁𝗿𝗼̀ 𝗰𝗵𝗼̛𝗶 𝗸𝗵𝗼̛̉𝗶 𝗰𝗵𝗮̣𝘆 𝘀𝗮𝘂',
