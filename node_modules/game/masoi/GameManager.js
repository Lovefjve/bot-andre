	...
	run(name, gameOptions) {
		...
		for (const item of this.items) {
			...
			if (item.threadID == gameOptions.threadID) {
				...
				return sendMessage(`𝗫𝗶𝗻 𝗹𝗼̂̃𝗶, 𝗯𝗼𝘅 𝗰𝘂̉𝗮 𝗯𝗮̣𝗻 𝗵𝗶𝗲̣̂𝗻 𝗰𝗵𝗮̣𝘆 𝘁𝗿𝗼̀ 𝗰𝗵𝗼̛𝗶 𝗿𝗼̂̀𝗶 ${item.name}!`, gameOptions.threadID);
			}	
			if (item.participants && item.participants.includes(gameOptions.masterID)) {
				return sendMessage(`𝗫𝗶𝗻 𝗹𝗼̂̃𝗶, 𝗯𝗮̣𝗻 𝗵𝗶𝗲̣̂𝗻 𝘁𝗿𝗼𝗻𝗴 𝘁𝗿𝗼̀ 𝗰𝗵𝗼̛𝗶 𝗿𝗼̂̀𝗶 ${item.name}!`, gameOptions.threadID);
			}
		}
